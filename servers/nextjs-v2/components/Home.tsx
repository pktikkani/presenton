...
export default function Home() {
    ...
    const fetchOllamaModelsWithConfig = async (config: any) => {
        try {
            const response = await fetch('/api/v2/ppt/ollama/list-supported-models');
    ...
    const pullOllamaModels = async (): Promise<void> => {
        return new Promise((resolve, reject) => {
            const interval = setInterval(async () => {
                try {
                    const response = await fetch(`/api/v2/ppt/ollama/pull-model?name=${llmConfig.OLLAMA_MODEL}`);
    ...
    const fetchCustomModels = async () => {
        try {
            ...
            const response = await fetch('/api/v2/ppt/models/list/custom', {
                method: 'POST',
                headers: {
